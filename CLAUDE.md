...
- **`Product.compatibleModels`** (`String?`) — the bikes a product fits, stored as the comma-separated labels (`"Pulsar N250 Dual ABS 2022 23, Pulsar N160 Dual ABS 2022 23"`). An assembly carries exactly one (that *is* its identity: two "Spark Plugs" groups differ only by bike); a loose part carries every bike that uses it, and **that is the cross-compatibility**: one brake pad serving the N250 and the N160 is a single row, not two. 59% of parts cover >=2 bikes. There was an attempt to type it with the `MotoModel` enum (`Product.models`) and **it was reverted in e2ff02d: that column is dropped and does not exist in the database** — the enum forced a migration per new bike, and the catalog grows (the Boxer BM150 arrived later and broke it). `MotoModel` survives only for `ScrapedProduct.model`, where the scrape fixes the value. Writing `models` from application code is therefore a build failure, not a runtime one: `prisma generate` runs in Heroku's postinstall and `next build` typechecks against the regenerated client — which is exactly how it slipped past for a while, since a stale local client still had the field. **`lib/modelo.ts`** is the presentation table (family, variant, years) and the only translation between that text and anything displayed; `lib/catalog.ts` exposes `whereModel()`, which filters with a `contains` on the label. Display goes through `formatModels`, which collapses by family — "N160 Single y Dual ABS", "N250 (todas)" — because thirteen full labels is 400 unreadable characters.
